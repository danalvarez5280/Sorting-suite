...
describe('Merge Sort', () => {
  ...
  it('should sort letters!', () => {
    ...
    expect(mergeAlpha).to.deep.equal(
          ['a', 'b', 'c', 'd', 'e', 'f', 'g', 'h', 'i', 'j', 'k', 'l', 'm']);
  });

  it('it now will sort the other half ot the alphabet', () => {
    let lettersArray2 = [
      'q', 'w', 'r', 't', 'y', 'u', 'o', 'p', 's', 'z', 'x', 'v', 'n'];
    let mergeAlpha2 = mergeSort(lettersArray2);

    expect(mergeAlpha2).to.deep.equal(
          ['n', 'o', 'p', 'q', 'r', 's', 't', 'u', 'v', 'w', 'x', 'y', 'z']);
  });

  it('merge sort can sort a lot of random numbers', () => {
    let randomMergeArray = getRandomNums(140000);
